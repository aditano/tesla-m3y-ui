...
| --- | --- | --- | --- | --- |
| Parked home | `screenshots/parked-home.png` | `manual-m3-touchscreen-p8.jpg`, `nata-parked-car-vis.jpg`, `nata-status-bar.jpg` | PARTIAL | Higher rear three-quarter, whole car in frame, light dock. Status order: lock, profile, Sentry, cellular, Wi-Fi, time, outdoor temp, “Passenger airbag”. Runtime mesh is the Studio Highland import. See `MESH_REVIEW.md`. |
...
| Parked full-screen vehicle | UI v12 notes + `nata-parked-car-vis.jpg` | PARTIAL | Scene `parked-home` loads the Studio Highland mesh in Ultra Red. David_Holiday aero-wheel, cabin-blocker, and greenhouse-split hacks are not applied. See `LICENSE-3D.md` + `MESH_REVIEW.md`. |
