...
| 7 | Media shortcut | Swipe min/max player | Dock card | partial — minimized strip on viz; tap expands translucent player | chrome |
...
| --- | --- | --- |
| Popup from dock arrows | Main climate, seat heat/vent, front/rear defrost, temp slider, Split | partial popup (main climate / seats / defrost / Split slider) |
...
| v12 player | Album art, title, thick scrubber with thumb, shuffle/repeat/search/EQ on the card, translucent over viz | partial translucent on-viz player |
| Minimized | One-line strip **in the visualization**, not a dock icon | partial strip on viz |
...
| --- | --- | --- |
| Quick Controls | Lights Off/Parking/On/Auto, wipers Off/Auto/I–IIII, fold mirrors, child lock, window lock, steering/mirror heat, car wash, glovebox, display brightness | partial tiles vs nata-quick-controls |
| Dynamics (was Pedals & Steering) | Accel, regen, Stopping Mode, Steering Weight Light/Standard/Heavy | partial “Dynamics” |
| Charging | Charge limit, scheduled charge, ports | stub charge limit + port |
...
| Trips | Trip meters | stub Trip A/B |
...
| Visualization | Cameras → surrounding model; other vehicles, brake/turn lamps, lane color when Autosteer/FSD active, proximity rays | partial — blue lane, windowed blocks, car-shaped traffic with amber turn lamps (2026.14). Simulated, not camera perception |
